...
# Make sure we can import the new architecture modules when running as a script
PROJECT_ROOT = Path(__file__).resolve().parents[1]
SRC_DIR = PROJECT_ROOT / "src"
if str(SRC_DIR) not in sys.path:
    sys.path.insert(0, str(SRC_DIR))

# Import parser from new architecture
try:
    from w40k.infrastructure.rag.parser import parse_article_html
except Exception as e:
    print(
        "Failed to import w40k.infrastructure.rag.parser.parse_article_html:",
        e,
        file=sys.stderr,
    )
    sys.exit(2)
