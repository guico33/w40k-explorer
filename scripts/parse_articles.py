...
from src.database.models import ScrapingStatus
from src.rag.parser import parse_article_html, PARSER_VERSION
...
def parse_articles(
    ...
    # Confirm processing
    confirm = input(f"Parse {len(articles_to_parse):,} articles? (y/n): ").lower().strip()
    if confirm != 'y':
    ...
    with tqdm(total=len(articles_to_parse), desc="Parsing articles", unit="articles") as pbar:
        for i, article in enumerate(articles_to_parse):
            try:
                ...
                if updated_article:
                    ...
                    if (i + 1) % (batch_size // 4) == 0:  # Log every 25 articles in a batch of 100
                        print(f"✅ Parsed '{title}' - Lead: {lead_length} chars, Sections: {sections_count}")
                else:
                    stats["failed"] += 1
                ...
                if stats["failed"] <= 5:
                    print(f"   🔍 Debug info - Article ID: {article.id}, Content length: {len(article.get_html_content()) if hasattr(article, 'get_html_content') else 'unknown'}")
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="Force reparse all articles")
