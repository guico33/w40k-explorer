...
def cleanup_database(db_path: str = "data/articles.db", dry_run: bool = True):
    ...
    with next(db_manager.get_session()) as session:
        ...
        failed_count = len(
            [a for a in articles_to_delete if a.status == ScrapingStatus.FAILED]
        )
        skipped_count = len(
            [a for a in articles_to_delete if a.status == ScrapingStatus.SKIPPED]
        )
        pending_count = len(
            [a for a in articles_to_delete if a.status == ScrapingStatus.PENDING]
        )
        empty_count = len(
            [
                a
                for a in articles_to_delete
                if a.raw_html_compressed == b"" or a.content_hash == "" or a.title == ""
            ]
        )
    ...
    # Confirm deletion
    confirm = (
        input(f"\n⚠️  Delete {total_to_delete:,} incomplete entries? (y/n): ")
        .lower()
        .strip()
    )
    if confirm != "y":
