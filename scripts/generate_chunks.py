...
from w40k.infrastructure.database.models import Article, ScrapingStatus
from w40k.infrastructure.database.operations import ChunkDatabase
from w40k.infrastructure.rag.chunker import ChunkParams, chunk_article
...
def generate_chunks(
    ...
    if not force_rechunk:
        try:
            confirm = (
                input(
                    f"\nGenerate chunks for {len(articles_to_chunk):,} articles? (y/n): "
                )
                .lower()
                .strip()
            )
    ...
    with tqdm(
        total=len(articles_to_chunk), desc="Generating chunks", unit="articles"
    ) as pbar:
        for i, article in enumerate(articles_to_chunk):
            try:
                ...
                if force_rechunk and article.id is not None:
                    ...
                    if cleared_count > 0:
                        print(
                            f"🗑️  Cleared {cleared_count} existing chunks for {article.title}"
                        )
                ...
                if stats["failed"] <= 5:
                    print(
                        f"   🔍 Debug info - Article ID: {article.id}, Title: {article.title}"
                    )
            ...
            if (i + 1) % batch_size == 0:
                ...
                print(
                    f"📈 Processed {i + 1:,}/{len(articles_to_chunk):,} articles → {chunks_in_batch:,} chunks"
                )
    ...
    if stats["total_chunks_generated"] > 0:
        avg_chunks_per_article = stats["total_chunks_generated"] / max(
            1, stats["success"]
        )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force regenerate all chunks"
    )
    parser.add_argument(
        "--max", type=int, help="Maximum number of articles to process (for testing)"
    )
