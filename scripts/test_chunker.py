    ...
    for json_file in sorted(json_files):
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                stats["chunks_by_type"][block_type] = (
                    stats["chunks_by_type"].get(block_type, 0) + 1
                )

                # Track token statistics
                if chunk.token_count:
                    stats["token_stats"]["min"] = min(
                        stats["token_stats"]["min"], chunk.token_count
                    )
                    stats["token_stats"]["max"] = max(
                        stats["token_stats"]["max"], chunk.token_count
                    )
            ...
            if chunks:
                sample = chunks[0]
                section_path = (
                    json.loads(sample.section_path) if sample.section_path else []
                )
                print(
                    f"     Sample: {' > '.join(section_path) if section_path else 'Root'}"
                )
